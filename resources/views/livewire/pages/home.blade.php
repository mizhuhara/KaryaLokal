...
<!-- ══════════════════════════════════════════════════
        ...
        <div class="max-w-7xl mx-auto px-6 relative z-10">
            <div class="max-w-3xl mx-auto text-center">
                ...
                <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium mb-6"
                    ...
                    🎨 Marketplace Kerajinan Tangan Indonesia #1
                </div>
        ...
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
                ...
                <div class="text-center relative z-10">
                    <div class="w-24 h-24 mx-auto mb-5 rounded-3xl flex items-center justify-center text-4xl relative"
        ...
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
                <div>
                    <h2 class="kl-section-title flex items-center gap-2">
                        <span class="animate-kl-pulse">📍</span>
                        Produk Terdekat
                    </h2>
            ...
            @elseif (!empty($nearbyProducts))
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 kl-stagger">
                    @foreach ($nearbyProducts as $product)
                        <a href="{{ route('product-detail', $product->id) }}" wire:navigate class="kl-product-card">
                            <div class="overflow-hidden h-52">
                                @if ($product->primaryImage)
                                    <img src="{{ asset('storage/' . $product->primaryImage->image_path) }}"
                                         alt="{{ $product->name }}" class="kl-product-img" />
                                @else
                                    <div class="w-full h-52 bg-gradient-to-br from-orange-50 to-orange-100 flex items-center justify-center">
                                        <span class="text-5xl opacity-30">🎨</span>
                                    </div>
                                @endif
                            </div>
                            <div class="p-4">
                                <h3 class="font-semibold text-gray-800 mb-1 line-clamp-2 font-jakarta">{{ $product->name }}</h3>
                                <p class="text-lg font-bold mb-2" style="color: #E8531D">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                                <div class="flex items-center justify-between">
                                    <p class="text-xs text-gray-500 truncate">{{ $product->sellerProfile->shop_name ?? 'Toko' }}</p>
                                    <span class="kl-badge kl-badge-orange">📍 {{ number_format($product->distance, 1) }} km</span>
                                </div>
                            </div>
                        </a>
                    @endforeach
            ...
            @else
                ...
                <div class="py-16 text-center rounded-2xl" style="background: linear-gradient(135deg, #FFFBF7, #FFF5F2); border: 2px dashed rgba(232,83,29,0.15)">
                    <div class="text-5xl mb-4 animate-kl-pulse">📍</div>
    ...
    <div class="max-w-7xl mx-auto px-6 py-16">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 kl-stagger">
            @foreach ($featuredProducts as $product)
                <a href="{{ route('product-detail', $product->id) }}" wire:navigate class="kl-product-card animate-fade-in-up">
                    <div class="overflow-hidden h-52">
                        @if ($product->primaryImage)
                            <img src="{{ asset('storage/' . $product->primaryImage->image_path) }}"
                                 alt="{{ $product->name }}" class="kl-product-img" />
                        @else
                            <div class="w-full h-52 bg-gradient-to-br from-orange-50 to-amber-50 flex items-center justify-center">
                                <span class="text-5xl opacity-30">🎨</span>
                            </div>
                        @endif
                    </div>
                    <div class="p-4">
                        <h3 class="font-semibold text-gray-800 mb-1 line-clamp-2 font-jakarta">{{ $product->name }}</h3>
                        <p class="text-lg font-bold mb-3" style="color: #E8531D">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                        <div class="flex items-center justify-between">
                            <p class="text-xs text-gray-500 truncate">🏪 {{ $product->sellerProfile->shop_name ?? 'Toko' }}</p>
                            <div class="flex gap-1">
                                ...
                                @if ($product->is_ready_stock)
                                    <span class="kl-badge kl-badge-green">Ready</span>
                                @endif
                            </div>
                        </div>
                    </div>
    ...
    <div class="py-16" style="background: linear-gradient(to bottom, #FFF5F2, white)">
        <div class="max-w-7xl mx-auto px-6">
            <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
                <div>
                    <h2 class="kl-section-title">🔥 Sedang Trending</h2>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 kl-stagger">
                @foreach ($trendingProducts as $i => $product)
                    <a href="{{ route('product-detail', $product->id) }}" wire:navigate
                       class="kl-product-card animate-fade-in-up relative">
                        <!-- Trending rank badge -->
                        ...
                        <div class="overflow-hidden h-52">
                            @if ($product->primaryImage)
                                <img src="{{ asset('storage/' . $product->primaryImage->image_path) }}"
                                     alt="{{ $product->name }}" class="kl-product-img" />
                            @else
                                <div class="w-full h-52 bg-gradient-to-br from-red-50 to-orange-50 flex items-center justify-center">
                                    <span class="text-5xl opacity-30">🔥</span>
                                </div>
                            @endif
                        </div>
                        <div class="p-4">
                            <h3 class="font-semibold text-gray-800 mb-1 line-clamp-2 font-jakarta">{{ $product->name }}</h3>
                            <p class="text-lg font-bold mb-2" style="color: #E8531D">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                            <p class="text-xs text-gray-500">🏪 {{ $product->sellerProfile->shop_name ?? 'Toko' }}</p>
                        </div>
                    </a>
                @endforeach
            </div>
    ...
    <div class="bg-white border-y py-16" style="border-color: #F0E8E0">
        <div class="max-w-7xl mx-auto px-6">
            <div class="text-center mb-10">
                <h2 class="kl-section-title">⭐ Pengrajin Unggulan</h2>
                    ...
                    <a href="{{ route('seller-store', $seller->id) }}" wire:navigate
                       class="kl-card p-6 group animate-fade-in-up">
                        <div class="flex items-start justify-between mb-4">
                            ...
                            <span class="kl-badge kl-badge-green">✅ Verified</span>
                        </div>
    ...
    <div class="max-w-7xl mx-auto px-6 py-16">
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
            <div>
                <h2 class="kl-section-title">💡 Rekomendasi Untuk Anda</h2>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 kl-stagger">
            @foreach ($recommendedProducts as $product)
                <a href="{{ route('product-detail', $product->id) }}" wire:navigate
                   class="kl-product-card animate-fade-in-up">
                    <div class="overflow-hidden h-40">
                        @if ($product->primaryImage)
                            <img src="{{ asset('storage/' . $product->primaryImage->image_path) }}"
                                 alt="{{ $product->name }}" class="kl-product-img" />
                        @else
                            <div class="w-full h-40 bg-gradient-to-br from-purple-50 to-pink-50 flex items-center justify-center">
                                <span class="text-3xl opacity-40">💡</span>
                            </div>
                        @endif
                    </div>
                    <div class="p-3">
                        <h3 class="font-semibold text-gray-800 text-sm mb-1 line-clamp-2 font-jakarta">{{ $product->name }}</h3>
                        <p class="font-bold text-sm" style="color: #E8531D">Rp {{ number_format($product->price, 0, ',', '.') }}</p>
                        <p class="text-xs text-gray-400 mt-1 truncate">{{ $product->sellerProfile->shop_name ?? 'Toko' }}</p>
                    </div>
                </a>
            @endforeach
        </div>
    ...
    <div class="py-16" style="background: #FFFBF7">
        <div class="max-w-7xl mx-auto px-6">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
                ...
                <div class="text-center">
                    <div class="w-16 h-16 mx-auto mb-3 rounded-2xl flex items-center justify-center text-3xl"
                         style="background: linear-gradient(135deg, #FDF2F8, #FCE7F3)">💬</div>
